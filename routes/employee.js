    ...
    Employee.queryEmployee(phone,function (err,result) {
        if (!err){
            if (_.isEmpty(result)){
                ...
                userId=result[0].id;
                Employee.bind(userId,wechatId,company,function (err,result) {
                    if (err){
                        let msg="手机号："+phone+" 绑定失败!";
                        unit.err(msg);
                        unit.showError(res,'绑定失败',err);
                    }else{
                        let msg="员工："+result.name+"绑定成功！";
                        unit.log(msg);
                        // 绑定成功后，转向邀约和被约信息界面
                        let param={};
                        let data={};
